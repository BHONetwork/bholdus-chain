...
fn multi_currency_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(BholdusTokens::mint(Origin::signed(ALICE), X_TOKEN_ID, BOB, 100));
...
fn multi_currency_should_not_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		PalletBalances::make_free_balance_be(&ALICE, 100);
		assert_ok!(BholdusTokens::create(Origin::signed(ALICE), ALICE, 1));
		assert_ok!(BholdusTokens::mint(Origin::signed(ALICE), X_TOKEN_ID, ALICE, 100));
		assert_ok!(BholdusTokens::freeze_asset(Origin::signed(ALICE), X_TOKEN_ID));
		assert_noop!(
