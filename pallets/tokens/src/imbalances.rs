...
/// accounting.
#[must_use]
pub struct PositiveImbalance<T: Config, GetCurrencyId: Get<T::AssetId>>(
	T::Balance,
	marker::PhantomData<GetCurrencyId>,
);

impl<T: Config, GetCurrencyId: Get<T::AssetId>> PositiveImbalance<T, GetCurrencyId> {
...
impl<T: Config, GetCurrencyId: Get<T::AssetId>> Default for PositiveImbalance<T, GetCurrencyId> {
	fn default() -> Self {
		Self::zero()
	}
}
...
#[must_use]
pub struct NegativeImbalance<T: Config, GetCurrencyId: Get<T::AssetId>>(
	T::Balance,
	marker::PhantomData<GetCurrencyId>,
);

impl<T: Config, GetCurrencyId: Get<T::AssetId>> NegativeImbalance<T, GetCurrencyId> {
...
impl<T: Config, GetCurrencyId: Get<T::AssetId>> Default for NegativeImbalance<T, GetCurrencyId> {
	fn default() -> Self {
		Self::zero()
	}
}

impl<T: Config, GetCurrencyId: Get<T::AssetId>> TryDrop for PositiveImbalance<T, GetCurrencyId> {
	fn try_drop(self) -> result::Result<(), Self> {
		self.drop_zero()
	}
}

impl<T: Config, GetCurrencyId: Get<T::AssetId>> Imbalance<T::Balance>
...
impl<T: Config, GetCurrencyId: Get<T::AssetId>> TryDrop for NegativeImbalance<T, GetCurrencyId> {
	fn try_drop(self) -> result::Result<(), Self> {
		self.drop_zero()
	}
}
...
// impl<T: Config, GetCurrencyId: Get<T::AssetId>> Drop for PositiveImbalance<T, GetCurrencyId> {
...
// impl<T: Config, GetCurrencyId: Get<T::AssetId>> Drop for NegativeImbalance<T, GetCurrencyId> {
//     /// Basic drop handler will just square up the total issuance.
//     fn drop(&mut self) {
