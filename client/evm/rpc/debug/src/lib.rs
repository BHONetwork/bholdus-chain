...
pub use bholdus_evm_rpc_core_debug::{Debug as DebugT, DebugServer, TraceParams};
...
use bholdus_client_evm_tracing::{formatters::ResponseFormatter, types::single};
use bholdus_evm_rpc_core_types::{RequestBlockId, RequestBlockTag};
use bholdus_evm_rpc_primitives_debug::{DebugRuntimeApi, TracerInput};
	...
	C::Api: EthereumRuntimeRPCApi<B>,
{
	...
	fn handle_block_request(
		...
		let statuses = api.current_transaction_statuses(&reference_id).map_err(|e| {
			internal_err(format!(
		...
		let eth_tx_hashes = statuses.unwrap().iter().map(|t| t.transaction_hash).collect();
		...
		let f = || -> RpcResult<_> {
			...
			Ok(bholdus_evm_rpc_primitives_debug::Response::Block)
		};

		return match trace_type {
			single::TraceType::CallList => {
				let mut proxy = bholdus_client_evm_tracing::listeners::CallList::default();
				proxy.using(f)?;
				proxy.finish_transaction();
				let response = match tracer_input {
					TracerInput::CallTracer =>
						bholdus_client_evm_tracing::formatters::CallTracer::format(proxy)
		...
		if let Some(block) = reference_block {
			...
			if let Some(transaction) = transactions.get(index) {
				let f = || -> RpcResult<_> {
					...
					Ok(bholdus_evm_rpc_primitives_debug::Response::Single)
				};

				return match trace_type {
					single::TraceType::Raw { disable_storage, disable_memory, disable_stack } => {
						let mut proxy = bholdus_client_evm_tracing::listeners::Raw::new(
							disable_storage,
							disable_memory,
							disable_stack,
						);
						proxy.using(f)?;
						Ok(Response::Single(
							bholdus_client_evm_tracing::formatters::Raw::format(proxy).unwrap(),
						))
					},
					single::TraceType::CallList => {
						let mut proxy = bholdus_client_evm_tracing::listeners::CallList::default();
						...
						let response = match tracer_input {
							TracerInput::Blockscout =>
								bholdus_client_evm_tracing::formatters::Blockscout::format(proxy)
									.ok_or("Trace result is empty.")
									.map_err(|e| internal_err(format!("{:?}", e))),
							TracerInput::CallTracer => {
								let mut res =
									bholdus_client_evm_tracing::formatters::CallTracer::format(
										proxy,
									)
									.ok_or("Trace result is empty.")
									.map_err(|e| internal_err(format!("{:?}", e)))?;
