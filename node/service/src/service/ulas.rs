...
pub use ulas_runtime::RuntimeApi;
...
pub fn create_extrinsic(
    ...
    function: impl Into<ulas_runtime::Call>,
    nonce: Option<u32>,
) -> ulas_runtime::UncheckedExtrinsic {
    ...
    let period = ulas_runtime::BlockHashCount::get()
        .checked_next_power_of_two()
        .map(|c| c / 2)
        .unwrap_or(2) as u64;
    let tip = 0;
    let extra: ulas_runtime::SignedExtra = (
        frame_system::CheckSpecVersion::<ulas_runtime::Runtime>::new(),
        frame_system::CheckTxVersion::<ulas_runtime::Runtime>::new(),
        frame_system::CheckGenesis::<ulas_runtime::Runtime>::new(),
        frame_system::CheckEra::<ulas_runtime::Runtime>::from(generic::Era::mortal(
            period,
            best_block.saturated_into(),
        )),
        frame_system::CheckNonce::<ulas_runtime::Runtime>::from(nonce),
        frame_system::CheckWeight::<ulas_runtime::Runtime>::new(),
        pallet_transaction_payment::ChargeTransactionPayment::<ulas_runtime::Runtime>::from(tip),
    );

    let raw_payload = ulas_runtime::SignedPayload::from_raw(
        function.clone(),
        extra.clone(),
        (
            ulas_runtime::VERSION.spec_version,
            ulas_runtime::VERSION.transaction_version,
            genesis_hash,
            best_hash,
            (),
            (),
    ...
    ulas_runtime::UncheckedExtrinsic::new_signed(
        function.clone(),
        sp_runtime::AccountId32::from(sender.public()).into(),
        ulas_runtime::Signature::Sr25519(signature.clone()),
        extra.clone(),
    )
}
    ...
    let (rpc_extensions_builder, rpc_setup) = {
        ...
        let rpc_extensions_builder =
            move |deny_unsafe, subscription_executor: bholdus_rpc::SubscriptionTaskExecutor| {
                let deps = bholdus_rpc::ulas::FullDeps {
                    client: client.clone(),
                    pool: pool.clone(),
                ...
                bholdus_rpc::ulas::create_full(deps).map_err(Into::into)
            };
