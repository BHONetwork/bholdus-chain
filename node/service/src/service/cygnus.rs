...
pub use cygnus_runtime::RuntimeApi;
...
pub fn create_extrinsic(
    ...
    function: impl Into<cygnus_runtime::Call>,
    nonce: Option<u32>,
) -> cygnus_runtime::UncheckedExtrinsic {
    ...
    let period = cygnus_runtime::BlockHashCount::get()
        .checked_next_power_of_two()
        .map(|c| c / 2)
        .unwrap_or(2) as u64;
    let tip = 0;
    let extra: cygnus_runtime::SignedExtra = (
        frame_system::CheckSpecVersion::<cygnus_runtime::Runtime>::new(),
        frame_system::CheckTxVersion::<cygnus_runtime::Runtime>::new(),
        frame_system::CheckGenesis::<cygnus_runtime::Runtime>::new(),
        frame_system::CheckEra::<cygnus_runtime::Runtime>::from(generic::Era::mortal(
            period,
            best_block.saturated_into(),
        )),
        frame_system::CheckNonce::<cygnus_runtime::Runtime>::from(nonce),
        frame_system::CheckWeight::<cygnus_runtime::Runtime>::new(),
        pallet_transaction_payment::ChargeTransactionPayment::<cygnus_runtime::Runtime>::from(tip),
    );

    let raw_payload = cygnus_runtime::SignedPayload::from_raw(
        function.clone(),
        extra.clone(),
        (
            cygnus_runtime::VERSION.spec_version,
            cygnus_runtime::VERSION.transaction_version,
            genesis_hash,
            best_hash,
            (),
            (),
    ...
    cygnus_runtime::UncheckedExtrinsic::new_signed(
        function.clone(),
        sp_runtime::AccountId32::from(sender.public()).into(),
        cygnus_runtime::Signature::Sr25519(signature.clone()),
        extra.clone(),
    )
}
    ...
    let (rpc_extensions_builder, rpc_setup) = {
        ...
        let rpc_extensions_builder =
            move |deny_unsafe, subscription_executor: bholdus_rpc::SubscriptionTaskExecutor| {
                let deps = bholdus_rpc::cygnus::FullDeps {
                    client: client.clone(),
                    pool: pool.clone(),
                ...
                bholdus_rpc::cygnus::create_full(deps).map_err(Into::into)
            };
