...
pub mod tracer {
	...
	impl EvmTracer {
		pub fn new() -> Self {
			Self { step_event_filter: bholdus_evm_primitives_ext::bholdus_ext::step_event_filter() }
		}
		...
		pub fn emit_new() {
			bholdus_evm_primitives_ext::bholdus_ext::call_list_new();
		}
	}
	...
	impl EvmListener for EvmTracer {
		...
		fn event(&mut self, event: evm::tracing::Event) {
			...
			bholdus_evm_primitives_ext::bholdus_ext::evm_event(message);
		}
	}
	...
	impl GasometerListener for EvmTracer {
		...
		fn event(&mut self, event: evm_gasometer::tracing::Event) {
			...
			let message = event.encode();
			bholdus_evm_primitives_ext::bholdus_ext::gasometer_event(message);
		}
	}
	...
	impl RuntimeListener for EvmTracer {
		...
		fn event(&mut self, event: evm_runtime::tracing::Event) {
			...
			let message = event.encode();
			bholdus_evm_primitives_ext::bholdus_ext::runtime_event(message);
		}
	}
}
