...
#[cfg(feature = "std")]
fn se_num2hex<S, T>(v: &T, serializer: S) -> Result<S::Ok, S::Error>
where
    S: serde::Serializer,
    T: serde_hex::SerHex<serde_hex::CompactPfx>,
{
    serde_hex::SerHex::<serde_hex::CompactPfx>::serialize(v, serializer)
}

#[cfg(feature = "std")]
fn se_bytes2hex<S>(v: &Bytes, serializer: S) -> Result<S::Ok, S::Error>
where
    S: serde::Serializer,
{
...
mod tests {
    ...
    const header_json_string: &str = r#"{
  ...
  "nonce": "0x0000000000000000"
    ...
    fn deserialize_bsc_header_should_work() {
        let header = serde_json::from_str::<BSCHeader>(header_json_string).unwrap();
    ...
    #[test]
    fn serialize_bsc_header_should_work() {
        let header = serde_json::from_str::<BSCHeader>(header_json_string).unwrap();

        assert_eq!(
            serde_json::to_string_pretty(&header).unwrap().as_str(),
            header_json_string
        );
    }
}
